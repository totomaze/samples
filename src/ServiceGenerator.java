...
public class ServiceGenerator {
  ...
  public static void setListeProxy(List<Proxy> listeProxy) {
    ServiceGenerator.listeProxy = listeProxy;
  }
  ...
  public static void setListeCriteria(List<Criteria> listeCriteria) {
    ServiceGenerator.listeCriteria = listeCriteria;
  }
  ...
  public static void setMultiOccurence(boolean multiOccurence) {
    ServiceGenerator.multiOccurence = multiOccurence;
  }
  public ServiceGenerator(String pathInput,String pathOutput, String packageName,String name,String suffixe){
  ...
  public static void main(String[] argv) throws Exception {
    generateServiceForRecherche();
  }
  
  public static void generateServiceForRecherche() throws Exception {
    //Getting table column, type and mandatory flag
    setMultiOccurence(true);  
    List<Proxy> listeDataProxy = gettingProxyFromFile(pathInput);
      setListeProxy(listeDataProxy);
      generateProxyClass(pathOutput, name, listeDataProxy);
      ...
      generateServiceClass(pathOutput, listeDataCriteria,listeDataProxy);
  ...
  public static void generateServiceForSynthesis() throws Exception {
    
    traitementSynthese=true;
      List<Proxy> listeDataProxy = gettingProxyFromFile(pathInput);
      setListeProxy(listeDataProxy);
      generateProxyClass(pathOutput, name, listeDataProxy);
      List<Criteria> listeDataCriteria = gettingCriteriaFromFile(pathInput);
      setListeCriteria(listeDataCriteria);
      setMultiOccurence(isMulti());
      extractionCompute(listeProxy);
      generateServiceClassForSynthesis(pathOutput, listeDataCriteria,listeDataProxy);
      if (containsSomme(listeDataProxy)){
        SumServiceGenerator sumServiceGenerator= new SumServiceGenerator(pathInput,pathOutput, packageName, name, suffixe,listeDataProxy);
        SumServiceGenerator.generateSumService();
      }
	...
	private static void generateServiceClass(String path, 
		...
		try {
			...
			out.append(declareServiceClassSQLFilter(dataListProxy));
			out.append(declareServiceClassSQLOrder(dataListProxy));
			...
			if (isMultiOccurence()){
			  out.append(declareServiceProcessFindByCriteria(dataList,dataListProxy));
			  out.append(declareServiceProcessCount());
			}else{
        out.append(declareServiceProcessFindByPrimaryKey(dataList,dataListProxy));	
	...
	public static void generateServiceClassForSynthesis(String path, 
	                                         List<Criteria> dataList,List<Proxy> dataListProxy) {
	  try {
	    File file = new File(path+name+"Service.java");
	    BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file),"UTF8"));
	    out.append(declarePackageForSynthesis());
	    out.append(declareServiceImportServiceClass(dataList));
	    out.append(declareServiceClassCommentary());
	    out.append(declareServiceClassDeclarationForSynthesis());
	    out.append(declareServiceClassSQLClause());
	    out.append(declareServiceClassSQLFilter(dataListProxy));
	    out.append(declareServiceClassSQLOrder(dataListProxy));
	    out.append(declareServiceNameVersionDescription());
  ...
  for (Proxy proxy: listeProxy){
    if (proxy.isLien()){
      sb.append("     proxy.set"+proxy.getNom()+"_URL(DonneesMetierLienUtils.getInstance().computeXXX);\n");
    }
  }
  ...
  //traitement du type de compute service
   ...
   private static String declare_ComposantMetierESO(){
	...
	private static void generateProxyClass(String path, String name, List<Proxy> listeDonneesProxy){
		
		try {
			File file = new File(path+name+"Proxy.java");
			BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file),"UTF8"));
			out.append(declarePackage());
			out.append(declareImportProxyClass(listeDonneesProxy));
			out.append(declareProxyClassCommentary(name));
			out.append(declareClasseDeclaration());
			for (Proxy donnee : listeDonneesProxy) {
				out.append(donnee.declareConstante());
				out.append("\n");				
			}
			for (Proxy donnee : listeDonneesProxy) {
			  if (donnee.isLien()){
		       out.append(donnee.declareGetterURLProxy());
		        out.append(donnee.declareSetterURLProxy());
			  }
	...
	private static String declareImportProxyClass(List<Proxy> dataList) {
		StringBuilder sb = new StringBuilder();
		sb.append("import sopra.core.annotation.constraints.MaxLengthConstraint;\n");
	...
	private static void generateCriteriaBeanClass(String path, 
	                                         List<Criteria> dataList) {
	  try {
	    File file = new File(path+name+"CriteriaBean.java");
	    BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file),"UTF8"));
	    ...
	    out.append(declareGetterSetterCriteria(dataList));

	    out.append("}//fin classe ").append(name).append("CriteriaBean");
	    out.close();
	  } catch (Exception e) {
	    //Exception non soulevée
	  } 
    ...
    return sb.toString();
	 ...
	 private static String declareCriteriaBeanClassCommentary() {
	  ...
	  private static String declareCriteriaBeanClassDeclaration() {
	    ...
	    sb.append("@ModelBean\n");
	    sb.append("public class ").append(name).append("CriteriaBean extends "+name+"ServiceCriteria {\n");
	    sb.append("  //Constantes\n");
	    sb.append("  /** Version extraite du gestionnaire de configuration */\n");
	    sb.append("  @SuppressWarnings(\"hiding\")\n");
	    sb.append("  public static final String CLASS_VERSION = \"$Revision: $\";\n");
	  ...
	  private static String declareGetterSetterCriteria(List<Criteria> dataList) {
      StringBuilder sb = new StringBuilder();
      
      for (Criteria critere : dataList) {
        if (critere.getListeDeroulante()!=null || critere.getPanier()!=null){
        ...
        //SETTER
          sb.append("  public void set"+UpperFirst(critere.getNom())+"("+critere.getType()+" value){\n");
          sb.append("    super.addCriterion("+critere.getNom().toUpperCase()+"_CRITERION_NAME, value);\n");
          sb.append("  }\n");
          
          
          if (critere.getComposantMetier()!=null){
            //sb.append("/**\n");
            //sb.append(" * "+critere.getComposantMetier().getType().toUpperCase()+"\n");
            //sb.append(" * \n");
            //sb.append(" * @param "+critere.getComposantMetier().getType().toUpperCase()+" valeur à positionner au sein de la propriété\n");
            //sb.append(" */\n");
            sb.append("public void set"+critere.getComposantMetier().getType().toUpperCase()+"("+critere.getComposantMetier().getType().toUpperCase()+"Criterion "+critere.getComposantMetier().getType().toUpperCase()+") {\n");
            sb.append("  if (isLoggingDebug()) {\n");
            sb.append("    logDebug(\"set"+critere.getComposantMetier().getType().toUpperCase()+"() <-- \" + "+critere.getComposantMetier().getType().toUpperCase()+");\n");
            sb.append("  }\n");
            sb.append("  //\n");
            sb.append("  super.addCriterion("+critere.getComposantMetier().getType().toUpperCase()+"_CRITERION_NAME, "+critere.getComposantMetier().getType().toUpperCase()+");\n");
            sb.append("}\n");
          }
